...
def list_carriers(page: int = 1, page_size: int = 50, db: Session = Depends(get_db)):
...
def list_routes(mode: str | None = None, page: int = 1, page_size: int = 50, db: Session = Depends(get_db)):
    query = db.query(Route)
    if mode:
        query = query.filter(Route.mode == mode)
    offset = (page - 1) * page_size
